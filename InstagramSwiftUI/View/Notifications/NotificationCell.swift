        ...
        HStack {
            if let user = viewModel.notification.user {
                NavigationLink(destination: ProfileView(user: user)) {
                    ...
                    Text(viewModel.notification.username).font(.system(size: 14, weight: .semibold)) + Text(" ") + Text(viewModel.notification.type.notificationMessage).font(.system(size: 15))
                }
            }
